...
from plugins import list_nouser_commands
...
async def start_handler(client: Client, message: Message) -> None:
    ...
    img_text = helper_handlers.image_text
    ...
    if len(message.command) == 1:
        ...
        await message.reply_photo(img_text,caption = start_text, reply_markup=buttons)
    else:
        hun_text = helper_handlers.hunter_text
        force_text = format_text_message(helper_handlers.force_text, user)
        if await helper_handlers.user_is_not_join(user.id):
            await message.reply_photo(hun_text,caption = force_text, reply_markup=user_buttons)
            return
 ...
 #           temp_msg = await message.reply("Silahkan Tunggu")
            for msg in msgs:
                if not msg.empty:
                    await msg.copy(
                    ...
                    await message.reply_text(
                        "<b>⚠️ ꜰɪʟᴇ ᴛɪᴅᴀᴋ ᴅᴀᴘᴀᴛ ᴅɪᴀᴋsᴇs. ʜᴀʀᴀᴘ ʟᴀᴘᴏʀᴋᴀɴ ᴋᴇᴘᴀᴅᴀ ᴀᴅᴍɪɴ. 🙏</b>",
                        reply_markup=ikb(helper_buttons.Contact)
                )
...
@Client.on_message(filters.private & filters.incoming & ~filters.command(list_nouser_commands))
async def useless(_,message: Message):
    user = message.from_user
#    img_text = helper_handlers.reply_text
    reply_text = format_text_message(helper_handlers.reply_text, user)
