...
async def batch_handler(client: Client, message: Message) -> None:
    ...
    async def ask_for_message_id(ask_msg: str) -> int:
        ...
        try:
            ask_message = await client.ask(
                ...
                user_id=user_id,
                timeout=45,
                reply_markup=ikb([[("🗄️ ᴅᴀᴛᴀʙᴀsᴇ ᴄʜᴀɴɴᴇʟ", database_ch_link, "url")]]),
            )
    ...
    try:
        ...
        await message.reply_text(
            encoded_data_url,
            quote=True,
            reply_markup=ikb([[("🔗 sʜᴀʀᴇ", share_encoded_data_url, "url")]]),
...
@authorized_users_only
async def genlink_handler(client: Client, message: Message) -> None:
    database_chat_id = config.DATABASE_CHAT_ID
    ...
    genlink_message_id = await ask_for_message_id("Genlink")
    if genlink_message_id is None:
        return


    # Encode data
    try:
        genlink_id = genlink_message_id * abs(database_chat_id)
        encoded_data = url_safe.encode_data(f"id-{genlink_id}")
        encoded_data_url = f"https://t.me/{client.me.username}?start={encoded_data}"
        share_encoded_data_url = f"https://t.me/share/url?url={encoded_data_url}"
