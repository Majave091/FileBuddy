...
async def update_start_text_msg(value: str) -> None:
    """
    Updates the start reply message in the database.
...
async def add_reply_text_msg(value: str) -> None:
    """
    Adds or updates the reply text message in the database.

    Args:
        value (str): The reply text message to set.
    """
    await database.add_value(int(BOT_ID), "REPLY_TEXT", value)


async def del_reply_text_msg() -> None:
    """
    Clears the reply text message from the database.
    """
    await database.clear_value(int(BOT_ID), "REPLY_TEXT")


async def get_reply_text_msg() -> str:
    """
    Retrieves the current reply text message from the database.

    Returns:
        str: The reply text message. Defaults to an empty string if not set.
    """
    doc = await database.get_doc(int(BOT_ID))
    return doc.get("REPLY_TEXT", [""])[0] if doc else ""


async def update_reply_text_msg(value: str) -> None:
    """
    Updates the reply text message in the database.

    This function first deletes the existing reply text message and then
    adds the new message.

    Args:
        value (str): The new reply text message to set.
    """
    await del_reply_text_msg()
    await add_reply_text_msg(value)
